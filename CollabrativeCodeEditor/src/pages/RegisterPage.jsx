...
// // 4. Uncomment this line to use your local image
// // import connectCallImage from "../assets/Image.png";

// const RegisterPage = () => {
//   // 5. Get 'user' and 'setUser' from context
...
//   const location = useLocation(); // 6. Get location

//   // 7. Redirect if user is already logged in
//   if (user) {
...
//     if (nameError || usernameError || !isPasswordValid) {
...
//       if (response.ok) {
//         // 8. Use toast and setUser
//         toast.success("Registration successful! Welcome.");
//         setUser(data.user);
//         // 9. Navigate back to intended page or home
//         const from = location.state?.from?.pathname || "/";
//         navigate(from, { replace: true });
//       } else {
//         // 10. Use toast for errors
...
//           <img
//             // --- Use your imported image here, with the placeholder as a fallback ---
//             src=
...
// --- MODIFIED --- (Added Mail icon)
import { Code2, User, Lock, Eye, EyeOff, Mail } from "lucide-react";
import PasswordStrengthBar from "../components/PasswordStrengthBar"; // Assumes you have this
import GoogleAuthButton from "../components/GoogleAuthButton"; // Assumes you have this
import { useDebounce } from "../utils/useDebounce"; // Imports your file
...
const RegisterPage = () => {
  ...
  // Handle username availability (no changes)
  useEffect(() => {
    if (!debouncedUsername || usernameError) {
      setIsUsernameAvailable(null);
      return;
  ...
  // Password validity check (no changes)
  useEffect(() => {
    const password = formData.password;
    setPasswordValidity({
      length: password.length >= 6,
