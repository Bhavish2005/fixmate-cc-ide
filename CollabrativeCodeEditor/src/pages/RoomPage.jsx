...
// import toast from "react-hot-toast"; // <-- correct import for hot-toast
// const RoomPage = () => {
...
//   // --- 1. DEFINE THE NAVIGATION HANDLER ---
//   const onBackButtonEvent = useCallback((e) => {
//     e.preventDefault();
//     // Re-push the current state to prevent navigation
...
//   // --- 3. ADD THE USE EFFECT TO SET UP THE TRAP ---
//   useEffect(() => {
//     // Push an initial state to "capture" the navigation
//     window.history.pushState(null, "", window.location.href);
...
//       window.removeEventListener("popstate", onBackButtonEvent);
//     };
//   }, [onBackButtonEvent]); // Add dependency

//   // 🧩 Socket-based state (Participants & Pages)
//   useEffect(() => {
...
//       setOwnerId(ownerId);
//     });
//     // Ask server for initial state on join
...
//     };
//     // We pass `cleanExit` via other functions, so it doesn't need to be
//     // a dependency for the socket handlers themselves.
//   }, [roomId, userName, userId, activePageId, navigate, call, client]);

//   // ----------------------------------------------------
//   // 🧠 END: Centralized Page State & Sockets
//   // ----------------------------------------------------
...
//   // ----------------------------------------------------
//   // 🧠 START: Page Handlers (Emit to Server)
//   // ----------------------------------------------------
//   const addNewPage = () => {
...
//       // react-hot-toast doesn't have toast.warn — using neutral toast
//       toast("Page closed.");
//     },
//     [roomId]
//   );
...
//   // 👇 Handle Leave Room Function
//   const handleLeaveRoom = async () => {
//     // cleanExit is called inside, which will remove the popstate listener
//     if (call) {
//       try {
//         await call.leave();
//       } catch (err) {
...
//             {/* Tabs */}
//             <div className="flex items-center bg-gray-800 border-b border-gray-700 px-2 py-1 overflow-x-auto">
//               {pages.map((page) => (
//                 <div
//                   key={page.id}
//                   className={`flex items-center px-3 py-1 rounded-t-md cursor-pointer mr-2 ${page.id === activePageId
...
const RoomPage = () => {
  ...
  // ⭐️ NEW: Start editing on double click
  const startEditingPageName = (pageId) => {
    setEditingPageId(pageId);
  };

  // ⭐️ NEW: Save name on Blur or Enter
            ...
            <div className="flex items-center bg-gray-800 border-b border-gray-700 px-2 py-1 overflow-x-auto">
              {pages.map((page) => (
                <div
                  ...
                  onClick={() => setActivePageId(page.id)}
                  // ⭐️ NEW: Double click to edit
                  onDoubleClick={() => startEditingPageName(page.id)}
                >
                  {editingPageId === page.id ? (
                    // ⭐️ NEW: Input field when editing
                    <input
                      type="text"
